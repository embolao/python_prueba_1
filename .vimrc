...
" manually load vim-plug the firs time
...
" Ack code search (requires ack or agg intalled in the system)
...
" Create a .vim.custom file in the directory you must want to customize.
...
augroup cline
    ...
    " shoe cursor line on window enter or when exit from insert mode
    au WinEnter,InsertEnter * set cursorline
augroup END
...
" and also open just eough folds (zv) to make the line in which the cursor
...
let g:notes_directories = ['~/Documentos/Notes']
